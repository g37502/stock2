...
def Input_data(file = 'text.csv'):
	...
	try:
		...
		sql2 = f'select code,DATE_FORMAT(date,"%Y-%m-%d") date,DATE_FORMAT(date,"%H:%i:%S") time,volume,nature from kline_lmint_sec_deal_original where DATE_FORMAT(date,"%Y-%m-%d")=\"{date}\" and code=\"{code}\"'
	...
	# print(code,date[0],type(data))
		...
		print('Input_data,重复任务或无任务')


if __name__ == '__main__':
	while True:
		Input_data(file='limit.csv')
		# s=pd.read_csv('limit.csv')
		# print(s.shape)
